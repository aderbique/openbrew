...
CATALOG_PATH="${1:-$ROOT/Setup/Catalogs/hops.catalog.json}"
...
if [ ! -f "$CATALOG_PATH" ]; then
  echo "Hop catalog not found: $CATALOG_PATH" >&2
  exit 1
fi

python3 "$ROOT/scripts/generate-hop-catalog-sql.py" "$CATALOG_PATH" |
  docker run --rm -i "$SQL_IMAGE" /opt/mssql-tools/bin/sqlcmd \
    -S "$SQL_SERVER" -U sa -P "$SQL_PASSWORD" -C -d "$SQL_DATABASE" -b -i /dev/stdin

echo "Imported hop catalog: $CATALOG_PATH"
