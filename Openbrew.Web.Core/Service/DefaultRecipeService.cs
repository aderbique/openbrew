...
namespace Openbrew.Web.Core.Service
{
	public class DefaultRecipeService : IRecipeService
	{
		...
		/// </summary>
		public void FinalizeRecipe(Recipe recipe, bool publishRequested)
		{
			if (recipe == null)
			{
			...
			// Every successful save is kept as an owner-visible recipe. Only complete,
			// explicitly publishable recipes appear in public feeds and search.
			recipe.IsActive = true;
			var isComplete = recipe.Fermentables.Any(x => x.Amount > 0) && recipe.Yeasts.Any();
			recipe.IsPublic = publishRequested && isComplete;
		...
		public void DeleteRecipe(Recipe recipe)
		{
			...
			// Recent photos and recipe lists can be cached; clear the shared cache entry so deleted recipes
			// stop showing up in the photostream right away.
			this.CachingService.Remove("RecentRecipes");
		}
