...
/* --------------------------------------------------- [ Theme ] ---------------------------------------------------------- */
function theme_onReady() {
    var storageKey = 'openbrew-theme-preference';
    var mediaQuery = window.matchMedia ? window.matchMedia('(prefers-color-scheme: dark)') : null;

    function getPreference() {
        var preference = 'system';
        try {
            preference = localStorage.getItem(storageKey) || preference;
        } catch (e) { }
        return preference === 'light' || preference === 'dark' || preference === 'system' ? preference : 'system';
    }

    function applyPreference(preference) {
        var isDark = preference === 'dark' || (preference === 'system' && mediaQuery && mediaQuery.matches);
        $('html')
            .attr('data-theme', isDark ? 'dark' : 'light')
            .attr('data-theme-preference', preference);

        $('[data-theme-choice]').each(function () {
            var isSelected = $(this).attr('data-theme-choice') === preference;
            $(this).toggleClass('is-selected', isSelected).attr('aria-pressed', isSelected ? 'true' : 'false');
        });

        var status = preference === 'system'
            ? 'Following your system preference (' + (isDark ? 'Dark' : 'Light') + ' mode).'
            : 'Using ' + (isDark ? 'Dark' : 'Light') + ' mode.';
        $('#theme-choice-status').text(status);
    }

    applyPreference(getPreference());

    $('[data-theme-choice]').on('click', function () {
        var preference = $(this).attr('data-theme-choice');
        try {
    ...
    if (mediaQuery) {
        ...
        else if (mediaQuery.addListener) mediaQuery.addListener(updateSystemTheme);
    }
}

/* --------------------------------------------------- [ General ] ---------------------------------------------------------- */
// The legacy homepage initializer was removed with the slider. Keep a no-op
// hook so shared startup can continue on the recipe builder and other pages.
...
function general_onReady() {
    ...
    $('.recipe-import').click(function () {
        $.colorbox({ html: '<iframe id="ImportBeerXmlIFrame" src="/ImportBeerXmlDialog" width="560" height="270" scrolling="no" />', opacity: .35, overlayClose: false, escKey: false, scrolling: false });
        return false;
    });
...
function common_onReady() {
    $('body').on('click', '.delete-brewsession', function (e) {
        $.colorbox({ html: '<div class="delete-confirmation"><h3>Are you sure you want to delete this?</h3><p>Once you delete a brew session there is no way to get it back.</p><div class="delete-confirmation-actions"><a class="button button_red" href="/brew/' + $(this).attr("data-brewsessionid") + '/delete">Yes, Delete</a><a class="button button_gray" href="#" onclick="$.colorbox.close(); return false;">No, don\'t delete</a></div></div>', opacity: .35, overlayClose: false, escKey: false, scrolling: false });
        return false;
    });
    
    $('body').on('click', '.delete-recipe', function (e) {
        $.colorbox({ html: '<div class="delete-confirmation"><h3>Are you sure you want to delete this?</h3><p>Once you delete a recipe there is no way to get it back.</p><div class="delete-confirmation-actions"><a class="button button_red" href="/recipe/' + $(this).attr("data-recipeid") + '/delete">Yes, Delete</a><a class="button button_gray" href="#" onclick="$.colorbox.close(); return false;">No, don\'t delete</a></div></div>', opacity: .35, overlayClose: false, escKey: false, scrolling: false });
        return false;
    });
    ...
    $('#tastingnotes').on('click', 'a[data-tastingnoteid]', function () {
        $.colorbox({ html: '<div class="delete-confirmation"><h3>Are you sure you want to delete this?</h3><p>Once you delete tasting notes there is no way to get them back.</p><div class="delete-confirmation-actions"><a class="button button_red" href="#" onclick="deleteTastingNote(' + $(this).attr("data-tastingnoteid") + '); $.colorbox.close(); return false;">Yes, Delete</a><a class="button button_gray" href="#" onclick="$.colorbox.close(); return false;">No, don\'t delete</a></div></div>', opacity: .35, overlayClose: false, escKey: false, scrolling: false });
        return false;
    });
...
function feedback_onReady() {
    ...
    if ($('#view-feedbackreceived').length > 0) {
        parent.$.colorbox.resize({ width: 650, height: 260 });
...
function photoUploadComplete(thumbnailUrl, detailUrl) {
    if (thumbnailUrl) {
        $('#RecipePhotoPreview').attr('src', thumbnailUrl);
        $('#RecipePhotoPreview').parent('a').attr('href', detailUrl || thumbnailUrl);
        $('#PhotoPopupTrigger').text('New Photo');
    }

    var autoCloseTimeout = window.setTimeout(function () { $.colorbox.close(); }, 250);
    $(document).one('cbox_closed', function () { window.clearTimeout(autoCloseTimeout); });
}
...
function settings_onReady() {
    if ($('#view-settings').length > 0) {
        ...
        $('#ChooseUsername').click(function (evt) {
            evt.preventDefault();
            $('#UsernameFieldToggle').hide();
            $('#Username').val('');
        ...
        $('#unlink-google-button').click(function () {
            $('#unlink-google-panel').removeAttr('hidden').slideDown();
            $('#unlink-google-password').focus();
        });

        $('#confirm-unlink-google').click(function () {
            var button = $(this), status = $('#unlink-google-status');
            button.prop('disabled', true);
            status.text('');
            $.post('/user/unlinkgoogle', {
                newPassword: $('#unlink-google-password').val(),
                confirmPassword: $('#unlink-google-confirm-password').val()
            }).done(function (response) {
                status.text(response.Message);
                if (response.Success) {
                    $('#unlink-google-panel, #unlink-google-button, #google-email-connection').hide();
                    $('#EmailAddress').prop('readonly', false).removeClass('google-managed-email').removeAttr('aria-describedby');
                }
            }).fail(function (xhr) {
                status.text(xhr.responseJSON && xhr.responseJSON.Message ? xhr.responseJSON.Message : 'Unable to unlink Google right now.');
